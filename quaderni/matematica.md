...
layout: page
title: Matematica
subtitle: "Dai numeri fondamentali alle strutture astratte: l'eleganza della logica e del linguaggio universale"
---

<!-- 1. GENERAZIONE AUTOMATICA DEI TAG DI MATEMATICA -->
<div style="margin-bottom: 35px; padding: 15px; background: #f8f9fa; border-left: 4px solid #1f365c; border-radius: 0 6px 6px 0;">
  ...
  <button onclick="filtraMatematica('all')" class="btn-filtro active" style="margin: 2px 5px; padding: 4px 10px; border: 1px solid #1f365c; background: #1f365c; color: white; border-radius: 4px; font-size: 0.85rem; cursor: pointer; font-weight: 600;">
    Tutti i post
  </button>

  <!-- Script Jekyll per isolare solo i tag di Matematica -->
  {% assign post_matematica = site.categories.matematica %}
  {% assign tag_matematica = "" | split: "," %}
  
  {% for post in post_matematica %}
    {% for tag in post.tags %}
      {% unless tag_matematica contains tag %}
        {% assign tag_matematica = tag_matematica | push: tag %}
      {% endunless %}
    {% endfor %}
  {% endfor %}

  {% for tag in tag_matematica %}
  <button onclick="filtraMatematica('{{ tag | slugify }}')" class="btn-filtro" style="margin: 2px 5px; padding: 4px 10px; border: 1px solid #1f365c; background: transparent; color: #1f365c; border-radius: 4px; font-size: 0.85rem; cursor: pointer;">
    #{{ tag }}
  </button>
  {% endfor %}
</div>

<!-- 2. ELENCO DEI POST DI MATEMATICA -->
<div class="posts-list">
  {% if post_matematica.size > 0 %}
    {% for post in post_matematica %}
    ...
    {% endfor %}
  {% else %}
    <!-- Messaggio provvisorio per Matematica -->
    <div style="text-align: center; margin: 40px auto; padding: 20px; background: #f8f9fa; border-radius: 6px;">
      <p style="color: #666; font-size: 1.1rem; margin: 0;">
        📐 Stiamo calcolando... I primi articoli di Matematica arriveranno a breve. Q.E.D.!
      </p>
    </div>
  {% endif %}
</div>
